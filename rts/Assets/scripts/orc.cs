...
public class orc : MonoBehaviour {
	...
	void animate_character(Vector3 futur, Vector3 current)
	{
		...
		if (get_angle(relatif) == 0) {
			animator.SetTrigger ("walk_up_orc");
		} else if (get_angle(relatif) == 45) {
			animator.SetTrigger ("walk_up_right_orc");
		} else if (get_angle(relatif) == 90) {
			animator.SetTrigger ("walk_right_orc");
		}else if (get_angle(relatif) == 135) {
			animator.SetTrigger ("walk_down_right_orc");
		}else if (get_angle(relatif) == 180) {
			animator.SetTrigger ("walk_down_orc");
		}else if (get_angle(relatif) == 225) {
			animator.SetTrigger ("walk_down_left_orc");
		}else if (get_angle(relatif) == 270) {
			animator.SetTrigger ("walk_left_orc");
		}else if (get_angle(relatif) == 315) {
			animator.SetTrigger ("walk_up_left_orc");
		}
	}
